...
def create_package(
    ...
    try:
        ...
        package = api_post("/tracking/package", payload)

        # Redirect to the tracking page for this package
        return RedirectResponse(
            url=f"/tracking/search?q={tracking_number}",
            status_code=303
        )
    except Exception as e:
        return templates.TemplateResponse(
...
def add_event_form(tracking_number: str, request: Request):
    """Show form to add an event to a package"""
    return templates.TemplateResponse(
...
def add_event(
    ...
    try:
        ...
        api_post(f"/tracking/package/{tracking_number}/event", payload)

        # Redirect back to tracking detail
        return RedirectResponse(
            url=f"/tracking/{tracking_number}",
            status_code=303
        )
    except Exception as e:
        return templates.TemplateResponse(
